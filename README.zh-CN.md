...
**当前状态：v0.14.0 已发布。** 10 个 CLI 命令，16 个平台，722 个测试，94.2% 覆盖率。
...
- **管控执行（Governed Execution）** — v0.13 主题"Control"，在 v0.12 "Trust" 之上构建 5 项新能力：
  - **生命周期投影（Lifecycle Projection）**（`ivy state`、`ivy state set`、`ivy state recover`）— 基于 Change 的检查点模型，而非独立状态。防止双数据源漂移。始终允许向后转换。
  - **决策协议（Decision Protocol）**（`ivy state --pending`）— 4 个核心决策点（DP-1 需求确认、DP-3 设计批准、DP-4 实施就绪、DP-8 归档确认）+ 3 个事件钩子。通过 project.yaml 的 `workflow.decision_protocol` 配置。
  - **预设工作流（Preset Workflows）**（`ivy workflow preset [--detect]`）— 3 个内置预设（full/hotfix/tweak），自动检测+文件数超阈值时提示升级。
  - **工作流证据（Workflow Evidence）**（`ivy workflow evidence [--check-archive]`、`ivy workflow archive`）— 转换理由+引用存入 transitionHistory。归档就绪检查确保完整证据链。通过 `ivy export --dimension workflow-evidence` JSON 导出。
  - **执行隔离（Execution Isolation）**（`ivy workflow archive --clean`）— Git worktree 提供器，用于并行代理执行。优雅降级至 `provider: none`。Docker/DevContainer 接口保留至 v0.14+。
- **探索模式（Explore Mode）**（`ivy explore`）— 只读模式，显示允许/禁止操作列表。
- **3 个新命令**：`ivy state`、`ivy workflow`、`ivy explore`（共 29 个命令）。
- **673+ 个通过测试**，覆盖 64 个测试文件。
