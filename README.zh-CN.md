...
IvyFlow（`ivyflow-cli`）是一个 CLI 工具，把 Skill、Rule、Git Hook 安装到 AI 编码平台（v0.10 支持 16 个平台），让 Agent 严格按照 **9 步开发工作流** 推进，而不是上来就写代码。
...
8. 写入 `.ivy/project.yaml`，含 `version: '0.10.0'`、`platforms[]`、`detected_platforms[]`、`analytics_enabled: false`、`project_knowledge`、`quality_gates` 和 `fingerprint` 配置段。
...
ivy release --change <name>                   # 打包已完成变更的制品（v0.9）
ivy export metrics                           # 导出项目数据为 JSON（v0.10）
...
- **21 个命令**：`init` / `status` / `validate` / `doctor` / `analytics` / `dashboard` / `suggest` / `review` / `check` / `explain` / `rules` / `archive` / `verify` / `fingerprint` / `release` / `export` / `uninstall` / `update`（含 `--adr`、`--memory` 扩展）。
- **TypeScript PreToolUse Guard** — `PreToolUseGuard` 类带类型化评估管线（全局阻断 → 阶段规则 → 归档保护）。三种决策类型：`allow` / `block` / `warn`。`PlatformHookAdapter` 接口带 3 个实现：Windsurf（JSON）、Cursor（JSON）、Gemini（CLI）。向后兼容 v0.7-v0.9 旧版 Hook 配置。
- **记忆模式冻结** — `.ivy/memory/schema.yaml` 含 5 种记录类型（decision、constraint、risk、fact、evidence）。`MemoryStore` 类：写入（校验 YAML + JSON 索引）、查询（多条件过滤）、ADR 视图、记忆概览。
- **ADR 视图** — `ivy dashboard --adr` 以 ADR 索引渲染决策记录。`ivy dashboard --memory` 显示按类型的计数概览。`ivy archive --adr` 在归档时生成详细 ADR 条目。
- **导出 API** — `ivy export metrics` 支持 `--pipe`（stdout JSON）、`--project`（多项目）、`--dimension`（changes/metrics/knowledge）。只读：绝不修改 `.ivy/` 状态。
- **社区模板** — GitHub Issue 模板（bug report、feature request）、PR 模板、RFC 模板（`docs/rfc/`）。CONTRIBUTING.md、CODE_OF_CONDUCT.md、SECURITY.md。
- **Gemini CLI PreToolUse Hook** — 从 Experimental 升级。Hook 命令从 shell 脚本改为 `ivy validate`。
- **517 个通过测试**，覆盖 49 个测试文件。
- **5 个阶段**：`open → design → build → verify → archive`。
- **16 个平台**（与 v0.9 一致）。
